...
def main():
    ...
    df_dg = pd.read_csv(disgenetfile, sep='\t', encoding = "ISO-8859-1")
    df_dg.head()
    ...
    dftop = df.loc[(df['num_genes']>9) & (df['num_genes'] <= 200)]
    print(dftop.shape)
    #
    dftop['enrichment'] = dftop[['gene', 'disease']].apply(lambda x: calculate_enrichment(*x), axis=1 )
