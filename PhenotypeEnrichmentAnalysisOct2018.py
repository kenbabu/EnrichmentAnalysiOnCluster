...
def calculate_enrichment(genes,disease, bg_set='GO_Biological_Process_2018'):
    try:
        logging.info("Start processing {}".format(disease))
        enr = gp.enrichr(gene_list= genes,
                    description='GO_Enrichment',
                    gene_sets = bg_set,
                    outdir='GO_HIGH_CLEANED/{}'.format(disease),
                    no_plot= True,
                    cutoff=0.5)
...
def main():
    ...
    ERROR_IDX = df_high.index[df_high['disease'] == 'C0424551'].tolist()[0]
